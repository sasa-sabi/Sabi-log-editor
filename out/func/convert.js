...
exports.makeDownloadFile = exports.makeDisplay = exports.convertMd = void 0;
...
// Markdownのコンバート
function convertMd(code, dictionary) {
    // コード（```コード```）の変換
    code = code.replace(/```([\s\S]*?)```/gm, (match, content) => {
        ...
        program = program.replace(/\-m(.*?)「(.*?)」/g, "%sabilog.true.memo%$1%$2%");
        program = program.replace(/\-p\s+(.*?)/g, "%sabilog.true.marker%$1%");
        program = program.replace(/\-(.*?)「(.*?)」/g, "%sabilog.true.balloon%$1%$2%");
        return ('\n\t\t</p>\n\t\t<pre class="code ' +
            title +
            '"><code>' +
            program +
...
function makeDownloadFile(pages, k, pageKey, artnum, pagenum, headertitle, headersent, imageurl, archivesent) {
    var convertedHtml = "";
